...
names(filtered_data) <- c("Genes", "1", "1 CV", "2", "2 CV", "3", "3 CV", "4", "4 CV")
...
#Only keep glucose rows that have the final genenames
filtered_data_glucose <- filtered_data_glucose[filtered_data_glucose$`Scan REF` %in% genenames,]

#convert to matrix for easy use of heatmap
filtered_data <- data.matrix(filtered_data)

#Keep list of glucose genes
genenames_glucose <- filtered_data_glucose$`Scan REF`


filtered_data_glucose <- filtered_data_glucose[,-1]

#Convert glucose values to matrix for easy use of heatmap
#filtered_data_glucose <- as.numeric(filtered_data_glucose[,2])
filtered_data_glucose <- data.matrix(filtered_data_glucose)

#Testing
print(filtered_data_glucose)
...
## Heatmap of Figure 2a - Glucose - fix error requiring 2 columns by merging

```{r generate heatmaps and print for glucose, echo=FALSE}
#Turn off warnings for readablility
options(warn = -1)
...
#Heatmap with legend 
heatmap.2(filtered_data_glucose,
  labRow = genenames_glucose,
  main = "Fig. 2a - Glucose",     # heat map title
  notecol="black",      # change font color of cell labels to black
  density.info="none",  # turns off density plot inside color legend
  trace="none",         # turns off trace lines inside the heat map
  margins =c(1,8),      # widens margins around plot
